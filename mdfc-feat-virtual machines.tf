locals {
  ...
  virtual_machine_policies = {
    ...
    mdfc-log-analytics-arc2-autoprovisioning = {
  ...
  virtual_machine_roles = {
    virtual-machines-va-role-1 = {
      name   = "Security Admin"
      policy = "mdfc-va-autoprovisioning"
    }
    virtual-machines-arc1-role-1 = {
      name   = "Contributor"
      policy = "mdfc-log-analytics-arc1-autoprovisioning"
    }
    virtual-machines-arc2-role-1 = {
      name   = "Contributor"
      policy = "mdfc-log-analytics-arc2-autoprovisioning"
    }
  }
}
...
data "azurerm_policy_definition" "vm_policies" {
  for_each = contains(var.mdfc_plans_list, "VirtualMachines") ? local.virtual_machine_policies : {}
...
resource "azurerm_subscription_policy_assignment" "vm" {
  ...
  parameters           = each.key == "mdfc-va-autoprovisioning" ? local.va_type : null
  ...
  depends_on = [
    azurerm_security_center_subscription_pricing.mdfc_plans["VirtualMachines"]
  ]
}

# Enabling vm extensions - Log Analytics for vm
resource "azurerm_security_center_auto_provisioning" "auto_provisioning" {
  count = contains(var.mdfc_plans_list, "VirtualMachines") ? 1 : 0
...
data "azurerm_role_definition" "vm_roles" {
  for_each = contains(var.mdfc_plans_list, "VirtualMachines") ? local.virtual_machine_roles : {}
