locals {
  ...
  sql_server_virtual_machines_enabled = contains(local.final_plans_list, "SqlServerVirtualMachines") && !contains(var.mdfc_plans_list, "VirtualMachines")
}
...
resource "azurerm_subscription_policy_assignment" "sql" {
  ...
  depends_on = [
    azurerm_security_center_subscription_pricing.mdfc_plans["SqlServerVirtualMachines"]
  ]
}
